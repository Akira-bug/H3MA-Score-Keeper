...
const UpdateMatch = () => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        const response = await axios.get(`http://192.168.56.11:8080/matches/${matchId}`);
  ...
  const handleClick = async (e) => {
    ...
    if (confirmation) {
      try {
        await axios.put(`http://192.168.56.11:8080/matches/${matchId}`, match);
        navigate('/');
      } catch (err) {
        console.error(err);
      }
  ...
  const handleDelete = async () => {
    ...
    if (confirmation) {
      try {
        await axios.delete(`http://192.168.56.11:8080/matches/${matchId}`);
        navigate('/');
      } catch (err) {
        console.error(err);
      }
